...
namespace {
    ...
    constexpr uint8_t PRESS_ADDR     = 0xF7;
    constexpr uint8_t TEMP_ADDR      = 0xFA;
    constexpr uint8_t HUM_ADDR       = 0xFD;
    constexpr uint8_t TEMP_DIG_ADDR  = 0x88;
    constexpr uint8_t PRESS_DIG_ADDR = 0x8E;
    constexpr uint8_t HUM_DIG_ADDR1  = 0xA1;
    constexpr uint8_t HUM_DIG_ADDR2  = 0xE1;
    constexpr uint8_t ID_ADDR        = 0xD0;

    constexpr uint8_t TEMP_DIG_LENGTH      =  6;
    constexpr uint8_t PRESS_DIG_LENGTH     = 18;
    constexpr uint8_t HUM_DIG_ADDR1_LENGTH =  1;
    constexpr uint8_t HUM_DIG_ADDR2_LENGTH =  7;
    constexpr uint8_t DIG_LENGTH           = 32;
    constexpr uint8_t SENSOR_DATA_LENGTH   =  8;
}


/****************************************************************/
BME280::BME280(const Settings& settings)
        : m_settings(settings), m_initialized(false)
{}
...
bool BME280::ReadChipID()
{
    ...
    if (id[0] == ChipModel::ChipModel_BME280) {
        m_chip_model = ChipModel::ChipModel_BME280;
    } else if (id[0] == ChipModel::ChipModel_BMP280) {
        m_chip_model = ChipModel::ChipModel_BMP280;
    } else {
        m_chip_model = ChipModel::ChipModel_UNKNOWN;
        return false;
    }
    return true;
}
...
void BME280::CalculateRegisters(uint8_t& ctrlHum, uint8_t& ctrlMeas, uint8_t& config)
{
    ...
    ctrlMeas = ((uint8_t) m_settings.tempOSR << 5)
            | ((uint8_t) m_settings.presOSR << 2) | (uint8_t) m_settings.mode;
    // config register. (config[7:5] = standby time, config[4:2] = filter, ctrl_meas[0] = spi enable.)
    config = ((uint8_t) m_settings.standbyTime << 5)
            | ((uint8_t) m_settings.filter << 2)
...
bool BME280::ReadData(int32_t data[SENSOR_DATA_LENGTH])
{
    ...
    if (m_settings.mode == Mode::Mode_Forced) {
        WriteSettings();
    }
...
float BME280::CalculateTemperature(int32_t raw, int32_t& t_fine, TempUnit unit)
{
    ...
    var2 = (((((raw >> 4) - ((int32_t) dig_T1))
            * ((raw >> 4) - ((int32_t) dig_T1))) >> 12) * ((int32_t) dig_T3)) >> 14;
    t_fine = var1 + var2;
    final = (t_fine * 5 + 128) >> 8;
    return unit == TempUnit::TempUnit_Celsius ?
            (final / 100.0) : (final / 100.0 * 9.0 / 5.0 + 32.0);
}
    ...
    var1 = (((((raw << 14) - (((int32_t) dig_H4) << 20)
            ...
            * (((((((var1 * ((int32_t) dig_H6)) >> 10)
                    * (((var1 * ((int32_t) dig_H3)) >> 11) + ((int32_t) 32768)))
                    >> 10) + ((int32_t) 2097152)) * ((int32_t) dig_H2) + 8192)
                    >> 14));
    var1 =
            (var1
                    - (((((var1 >> 15) * (var1 >> 15)) >> 7)
...
/****************************************************************/
void BME280::read(float& pressure, float& temp, float& humidity,
...
/****************************************************************/
BME280::ChipModel BME280::chipModel() const
{
    return m_chip_model;
}
