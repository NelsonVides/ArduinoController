...
/****************************************************************/
float EnvironmentCalculations::Altitude(float pressure, AltitudeUnit altUnit,
        float referencePressure, float outdoorTemp, TempUnit tempUnit)
{
    // Equation inverse to EquivalentSeaLevelPressure calculation.
    float altitude = NAN;
    if (!isnan(pressure) && !isnan(referencePressure) && !isnan(outdoorTemp)) {
        if (tempUnit != TempUnit::TempUnit_Celsius)
        ...
        if (altUnit != AltitudeUnit::AltitudeUnit_Meters)
            altitude *= 3.28084;
    }
    return altitude;
}
...
/****************************************************************/
float EnvironmentCalculations::AbsoluteHumidity(float temperature,
        float humidity, TempUnit tempUnit)
{
...
/****************************************************************/
int EnvironmentCalculations::HeatIndex(float temperature, float humidity,
        TempUnit tempUnit)
{
    float heatindex = NAN;
    bool metric = true;
    float hi[2][9] = { { -8.784695, 1.61139411, 2.338549, -0.14611605,
            -1.2308094 / 100, -1.6424828 / 100, 2.211732 / 1000, 7.2546 / 10000,
            -3.582 / 1000000 }, { -42.379, 2.04901523, 10.1433127, -0.22475541,
            -6.83783 / 1000, -5.481717 / 100, 1.22874 / 1000, 8.5282 / 10000,
            -1.99 / 1000000 } };
    ...
    if (!isnan(humidity) && !isnan(temperature) && humidity > 40
            && (metric ? temperature > 26.7 : temperature > 80)) {
        ...
        heatindex = +hi[metric ? 0 : 1][6] * temperature * temperature
                * humidity;
        heatindex = +hi[metric ? 0 : 1][7] * temperature * humidity * humidity;
        heatindex = +hi[metric ? 0 : 1][8] * temperature * temperature
                * humidity * humidity;
...
/****************************************************************/
float EnvironmentCalculations::EquivalentSeaLevelPressure(float altitude,
        float temp, float pres, AltitudeUnit altUnit, TempUnit tempUnit)
{
    float seaPress = NAN;
        ...
        TempUnit tempUnit)
{
    ...
    if (!isnan(temp) && !isnan(hum)) {
        if (tempUnit == TempUnit::TempUnit_Celsius) {
            dewPoint = 243.04
                    * (log(hum / 100.0) + ((17.625 * temp) / (243.04 + temp)))
                    / (17.625 - log(hum / 100.0)
                            - ((17.625 * temp) / (243.04 + temp)));
        } else {
            float ctemp = (temp - 32.0) * 5.0 / 9.0;

            dewPoint = 243.04
                    * (log(hum / 100.0) + ((17.625 * ctemp) / (243.04 + ctemp)))
                    / (17.625 - log(hum / 100.0)
                            - ((17.625 * ctemp) / (243.04 + ctemp)));
