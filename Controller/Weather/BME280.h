...
public:
    ...
    enum class TempUnit {
        TempUnit_Celsius, TempUnit_Fahrenheit
    };
    ...
    enum OSR {
        OSR_Off = 0, OSR_X1 = 1, OSR_X2 = 2, OSR_X4 = 3, OSR_X8 = 4, OSR_X16 = 5
    };

    enum Mode {
        Mode_Sleep = 0, Mode_Forced = 1, Mode_Normal = 3
    };
    ...
    enum Filter {
        Filter_Off = 0, Filter_2 = 1, Filter_4 = 2, Filter_8 = 3, Filter_16 = 4
    };

    enum SpiEnable {
        SpiEnable_False = 0, SpiEnable_True = 1
    };

    enum ChipModel {
        ChipModel_UNKNOWN = 0, ChipModel_BMP280 = 0x58, ChipModel_BME280 = 0x60
    };
    ...
    struct Settings {
        Settings(OSR _tosr = OSR::OSR_X1,
                 OSR _hosr = OSR::OSR_X1,
                 OSR _posr = OSR::OSR_X1,
                 Mode _mode = Mode::Mode_Forced,
                 StandbyTime _st = StandbyTime::StandbyTime_1000ms,
                 Filter _filter = Filter::Filter_Off,
                 SpiEnable _se = SpiEnable::SpiEnable_False)
              : tempOSR(_tosr), humOSR(_hosr), presOSR(_posr), mode(_mode), standbyTime(
                        _st), filter(_filter), spiEnable(_se)
        {}
        OSR tempOSR;
        OSR humOSR;
        OSR presOSR;
    ...
    float temp(TempUnit unit = TempUnit::TempUnit_Celsius);

    /////////////////////////////////////////////////////////////////
    /// Read the pressure from the BME280 and return a float with the
    /// specified unit.
    float pres(PresUnit unit = PresUnit::PresUnit_hPa);
...
private:
    ...
    void CalculateRegisters(uint8_t& ctrlHum, uint8_t& ctrlMeas,
            uint8_t& config);
    ...
    float CalculateTemperature(int32_t raw, int32_t& t_fine,
            TempUnit unit = TempUnit::TempUnit_Celsius);
    ...
    float CalculatePressure(int32_t raw, int32_t t_fine,
            PresUnit unit = PresUnit::PresUnit_hPa);
};
}
#endif // TG_BME_280_H
