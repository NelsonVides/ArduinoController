...
constexpr uint8_t LCD_DATA                = 1;
constexpr uint8_t FOUR_BITS               = 2;
 ...
 */
constexpr uint16_t HOME_CLEAR_EXEC = 2000;
...
*/
constexpr uint8_t BACKLIGHT_OFF = 0;
 ...
 */
enum class t_backlightPol {POSITIVE, NEGATIVE};
...
protected:
   ...
   // classes.
   uint8_t _displayfunction = LCD_2LINE;  // LCD_5x10DOTS or LCD_5x8DOTS, LCD_4BITMODE or
                                          // LCD_8BITMODE, LCD_1LINE or LCD_2LINE
   uint8_t _displaycontrol = LCD_DISPLAYON | LCD_CURSOROFF | LCD_BLINKOFF;   // LCD base control command LCD on/off, blink, cursor
                              // all commands are "ored" to its contents.
   uint8_t _displaymode = LCD_ENTRYLEFT | LCD_ENTRYSHIFTDECREMENT;;      // Text entry mode to the LCD
   uint8_t _numlines         = 16; // Number of lines of the LCD, initialized with begin()
   uint8_t _cols             =  0; // Number of columns in the LCD
   t_backlightPol _polarity  =  t_backlightPol::POSITIVE;   // Backlight polarity
