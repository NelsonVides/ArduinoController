...
namespace Power {
    constexpr auto ANALOG_MAX_VALUE = 0x3FF;
    constexpr auto DEFAULT_REFERENCE_CALIBRATION = 1126400L;
    constexpr auto INVALID_REFERENCE_CALIBRATION = 0xFFFFFFL;

    constexpr uint32_t mergeBytes(uint8_t hi, uint8_t mid, uint8_t low) {
        return ((((long) hi) << 16) | ((mid << 8) | ((low & 0xFF) & 0xFFFF)));
    }
